...
router.get('/sessions', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const limit = Math.min(parseInt(String(req.query.limit), 10) || DEFAULT_LIMIT, MAX_LIMIT);
        const offset = Math.max(0, parseInt(String(req.query.offset), 10) || 0);
        const sessions = await chat_history_service_1.ChatHistoryService.getUserSessions(userId, limit, offset);
        res.json({ sessions });
    }
    catch (err) {
...
router.post('/sessions', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { title, model } = req.body || {};
        const session = await chat_history_service_1.ChatHistoryService.createSession(userId, title, model);
        if (!session) {
            res.status(503).json({ error: 'Chat history is not available' });
            return;
        }
...
router.get('/sessions/:sessionId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { sessionId } = req.params;
        const session = await chat_history_service_1.ChatHistoryService.getSession(sessionId, userId);
        if (!session) {
...
router.put('/sessions/:sessionId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { sessionId } = req.params;
        const { title, model, metadata, is_archived } = req.body || {};
        const session = await chat_history_service_1.ChatHistoryService.updateSession(sessionId, userId, {
            title,
            model,
            metadata,
            is_archived,
...
router.delete('/sessions/:sessionId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const ok = await chat_history_service_1.ChatHistoryService.deleteSession(sessionId, userId, archiveOnly);
        if (!ok) {
            res.status(404).json({ error: 'Session not found' });
            return;
        }
...
router.get('/sessions/:sessionId/messages', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { sessionId } = req.params;
        const limit = Math.min(parseInt(String(req.query.limit), 10) || DEFAULT_LIMIT, MAX_LIMIT);
        const offset = Math.max(0, parseInt(String(req.query.offset), 10) || 0);
        const messages = await chat_history_service_1.ChatHistoryService.getSessionMessages(sessionId, userId, limit, offset);
        res.json({ messages });
    }
    catch (err) {
        if (err.message === 'Chat history database is not configured') {
...
router.get('/sessions/:sessionId/context', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { sessionId } = req.params;
        const maxTokens = Math.min(parseInt(String(req.query.max_tokens), 10) || 32000, 100000);
        const messages = await chat_history_service_1.ChatHistoryService.getSessionContext(sessionId, userId, maxTokens);
        res.json({ messages });
    }
    catch (err) {
        if (err.message === 'Chat history database is not configured') {
